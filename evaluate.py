...
def parse_args():
    ...
    parser.add_argument("--ckpt", type=str, help="xx.pt")
...
def main(cfg: DictConfig):
    ...
    assert args.ckpt.endswith(".pt"), "Checkpoint must be a .pt file"
    ...
    if train_cfg.ema is None:
        val_acc_dict = evaluate_ddp_dict(model, cfg, device, rank, world_size, step=0, logdir=track_dir)
    else:
        val_acc_dict = None

    # validation loss (mdm loss on the validation dataset)
    val_loss = val_loss_ddp(model, val_loader, mask_id, device, rank, world_size, cfg.training.strategy, eos_id, arm_init=model_config.predict_next_token)
