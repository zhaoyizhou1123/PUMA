...
def mdm_loss(model, input_ids, mask_id: int, prompt_mask: Optional[torch.Tensor] = None, arm_init: bool = False):
    ...
    loss = loss.sum() / B
    # print(f"num_mask: {num_mask[:, 0].cpu().tolist()}")
    return loss, logits.detach(), num_mask.detach()

    # # manually compute ce loss
    # logprobs = F.log_softmax(logits, dim=-1)
    # nll = -logprobs.gather(dim=-1, index=input_ids.unsqueeze(-1)).squeeze(-1)
    # per_seq_loss = (nll * mask_indices.float()).sum(dim=1, keepdim=True)
    # loss = (per_seq_loss / num_mask[:, :1]).sum() / B
    # print(f"predictor loss: num_mask {num_mask.mean().item()}, loss {loss.item()}, naive mean loss {(nll * mask_indices.float()).mean().item()}")
    # return loss, logits.detach(), num_mask.detach()
...
def proseco_loss(model, input_ids, logits: torch.Tensor, num_mask: torch.Tensor, mask_id: int, prompt_mask: Optional[torch.Tensor] = None, arm_init: bool = False):
    ...
    L_eff = L - prompt_mask.sum(dim=1 , keepdim=True).float().expand_as(input_ids)
    # # uniformly sample the number of positions to mask
    # num_mask = torch.floor(torch.rand(B, 1, device=device) * L_eff.clamp(min=1)).long() + 1
    ...
    # There's a bug here: We should not modify prompt
    with torch.no_grad():
    ...
    # Use L_eff instead of actual num masks to compare difference. This lowers correction loss
    correction_loss = correction_loss / L_eff[~prompt_mask]
...
def main(cfg: DictConfig):
    ...
    num_training_steps = train_cfg.num_epochs * len(train_loader) * 2 # two steps for correction and prediction
    print(f"Total training steps (including correction and prediction): {num_training_steps}")
