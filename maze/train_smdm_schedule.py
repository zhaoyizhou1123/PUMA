...
def evaluate_ddp(model, cfg, device, rank: int, world_size: int, sampling, step=0, logdir=None):
    if cfg.data.dataset == "sudoku":
        ...
        return evaluate_ddp_gsm8k_smdm(model, cfg, device, rank, world_size, sampling, step=step, logdir=logdir)
    else:
...
def mdm_loss(model, input_ids, mask_id: int, prompt_mask: Optional[torch.Tensor] = None, arm_init: bool = False):
    ...
    # print(f"avg loss: {loss.mean().item()}, avg num_mask: {num_mask[mask_indices].mean().item()}, avg loss per masked token: {(ce / num_mask[mask_indices]).mean().item()}")
    return loss.sum() / B

# validation loss helper
def val_loss_ddp(model, val_loader, mask_id: int, device, rank: int, world_size: int, strategy: str, eos_id: int):
    model.eval()
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, desc = "Validating", disable = (rank != 0)):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled = torch.cuda.is_available()):
                if strategy in ["progressive_edit", "edit_v2", "edit_v3", "edit_v3_2", "edit_v3_3", "edit_v4", "edit_v5", "edit_v6", "edit_v7"]:
                    loss = mdm_loss_edit(model, x0, mask_id, prompt_mask=pm)
...
def main(cfg: DictConfig):
    ...
    batch_size_per_gpu = cfg.training.get("batch_size_per_gpu", bs // world_size)
    assert (bs // world_size) % batch_size_per_gpu == 0, \
        f"per-GPU batch size {bs // world_size} must be divisible by batch_size_per_gpu {batch_size_per_gpu}"
    grad_accum_steps = (bs // world_size) // batch_size_per_gpu
    with open_dict(cfg):
        cfg.data.training.per_gpu_batch_size = batch_size_per_gpu
    if is_main:
        print(f"Gradient accumulation: batch_size_per_gpu={batch_size_per_gpu}, grad_accum_steps={grad_accum_steps}")
    ...
    if cfg.validation.get("track", False):
        track_dir = f"track/{cfg.wandb.project}/{cfg.wandb.name}/date{datetime_str}"
    ...
    # Optionally load a pretrained SMDM checkpoint
    pretrain_path = cfg.model.get("pretrain_path", None)
    if pretrain_path and pretrain_path != "none":
        if is_main:
            print(f"Loading SMDM pretrained checkpoint from {pretrain_path}")
        if pretrain_path.endswith(".safetensors"):
            sd = load_file(pretrain_path)
        else:
    ...
    if world_size > 1 and torch.cuda.is_available():
        ...
        train_loader = DataLoader(
            train_loader.dataset,
            batch_size=data_cfg.training.per_gpu_batch_size,
    ...
    max_position = cfg.data.get("max_len", smdm_config.block_size)
    ...
    for epoch in range(train_cfg.num_epochs):
        ...
        for step_in_epoch, itr in enumerate(pbar):
            ...
            for micro_step in range(grad_accum_steps):
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=torch.cuda.is_available()):
                    if strategy in ["progressive_edit", "edit_v2", "edit_v3", "edit_v3_2", "edit_v3_3", "edit_v4", "edit_v7"]:
                        ...
                        loss = mdm_edit_loss_fn(log_probs, pool.x0, pool.xt, mask_id, prompt_mask=pool.state['prompt_mask'])
                    elif strategy in ["edit_v5", "edit_v6"]:
                        xt = pool.current_batch()
                        logits = model(xt)
                        log_probs = F.log_softmax(logits, dim=-1)
                        loss = mdm_edit_loss_fn_v5(log_probs, pool.x0, pool.xt, mask_id, prompt_mask=pool.state['prompt_mask'])
            ...
            if is_main:
                pbar.set_postfix(loss=accum_loss, lr=optimizer.param_groups[0]["lr"])

                if global_step % train_cfg.logging_steps == 0:
                    print(f"Epoch {epoch+1}, Step {global_step}, Loss {accum_loss}")
                    if cfg.wandb.wandb:
                        wandb.log({"train/loss": accum_loss}, step=global_step)
            ...
            if global_step % train_cfg.eval_steps == 0:
                ...
                val_loss = val_loss_ddp(model, val_loader, mask_id, device, rank, world_size, strategy, eos_id)
