...
class SudokuVisualizer:
    ...
    def create_controls(self):
        ...
        self.load_npy_btn = tk.Button(top_frame, text="Load .npy", command=self.load_npy_file, width=10, bg="#e0e0e0")
        self.load_npy_btn.pack(side=tk.LEFT, padx=5)

        self.load_pkl_btn = tk.Button(top_frame, text="Load .pkl", command=self.load_pkl_file, width=10, bg="#d0e8ff")
        self.load_pkl_btn.pack(side=tk.LEFT, padx=5)
    ...
    def _finalize_load(self, full_data, filename):
        """Common logic after loading data into self.full_data (N, T, seq_len)."""
        self.full_data = full_data
        print("Loaded data shape (N, T, seq_len):", self.full_data.shape)
    ...
    def load_npy_file(self):
        ...
        try:
            ...
            # raw_data is (T, N, seq_len) — transpose to (N, T, seq_len)
            full_data = np.transpose(raw_data, (1, 0, 2))
            self._finalize_load(full_data, filepath.split('/')[-1])
        except Exception as e:
            messagebox.showerror("Error", f"Failed to load file:\n{e}")

    def load_pkl_file(self):
        filepath = filedialog.askopenfilename(
            title="Select Track PKL File",
        ...
        try:
            ...
            if not isinstance(data, dict) or 'track_xt' not in data:
                messagebox.showerror("Error", "PKL file must contain a dict with key 'track_xt'.")
                return

            track_xt = data['track_xt']  # torch.Tensor (N, T, seq_len)
            if isinstance(track_xt, torch.Tensor):
                full_data = track_xt.numpy()
            else:
                full_data = np.array(track_xt)
            # shape is (T, N, seq_len) e.g. (81, 1000, 162)

            # track_xt is (T, N, seq_len) — transpose to (N, T, seq_len)
            full_data = np.transpose(full_data, (1, 0, 2))
            self._finalize_load(full_data, filepath.split('/')[-1])
        except Exception as e:
            messagebox.showerror("Error", f"Failed to load file:\n{e}")
